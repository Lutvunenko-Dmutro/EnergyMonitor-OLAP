"""
ВІЗУАЛЬНИЙ ІНТЕРФЕЙС СЕГМЕНТАЦІЇ ТА КЛАСТЕРИЗАЦІЇ ОБ'ЄКТІВ (Clustering Visualization)
================================================================================
    ...
    try:
        ...
        if not df_grouped.empty:
            ...
            with col_stats:
                ...
                top_loaded_display = top_loaded[["substation_name", "avg_load"]].rename(
                    columns={
                        "substation_name": "Підстанція",
                        "avg_load": "Навантаження (МВт)"
                    }
                )
                st.dataframe(top_loaded_display, hide_index=True, use_container_width=True)
        else:
