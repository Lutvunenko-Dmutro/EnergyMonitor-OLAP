...
| **v2** | 5 (+ погода) | 24h | 2 | Архів |
| **v3** | 9 (+ time harmonics) | 48h | 2 | ✅ **Production** |
| **Zero-Fail** | — | — | 2 | Fallback |
...
for step in range(hours_ahead):          # 24 кроки вперед
    prediction = model.predict(window)   # LSTM.predict → (load_t+1, health_t+1)
...
Функція втрат — **Multi-output MSE**:
$$\mathcal{L} = \frac{1}{M \cdot 2}\sum_{i=1}^{M}\left[(Y_i^{load} - \hat{Y}_i^{load})^2 + (Y_i^{health} - \hat{Y}_i^{health})^2\right]$$
