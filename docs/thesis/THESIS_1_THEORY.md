...
$$y(t) = T(t) + S(t) + C(t) + \epsilon(t), \eqno (1.1)$$
...
$$f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f), \eqno (1.2)$$
2. **Input Gate (Гейт входу)**: 
$$i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i), \eqno (1.3)$$
$$\tilde{C}_t = \tanh(W_C \cdot [h_{t-1}, x_t] + b_C), \eqno (1.4)$$
3. **Update Cell State (Оновлення стану комірки)**: 
$$C_t = f_t * C_{t-1} + i_t * \tilde{C}_t, \eqno (1.5)$$
4. **Output Gate (Гейт виходу)**: 
$$o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + b_o), \eqno (1.6)$$
$$h_t = o_t * \tanh(C_t), \eqno (1.7)$$
