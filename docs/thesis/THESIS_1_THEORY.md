...
#### 1.2.2. Проблема зникаючого градієнта та архітектура LSTM
Чому традиційні RNN не завжди працюють? Традиційні рекурентні мережі страждають від проблеми зникаючого градієнта (**Vanishing Gradient**) [4, 9], що робить їх нездатними вивчати довгострокові залежності. Архітектура LSTM (Long Short-Term Memory) була запропонована для подолання цього бар'єру за допомогою механізму **гейтів (Gates)**.

**Математична модель комірки LSTM:**
1.  **Forget Gate:** $f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)$
2.  **Input Gate:** $i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)$ та $\tilde{C}_t = \tanh(W_C \cdot [h_{t-1}, x_t] + b_C)$
3.  **Update Cell State:** $C_t = f_t * C_{t-1} + i_t * \tilde{C}_t$
4.  **Output Gate:** $o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + b_o)$ та $h_t = o_t * \tanh(C_t)$

#### 1.2.3. Алгоритми оптимізації та навчання
Для мінімізації функції втрат обрано алгоритм **Adam (Adaptive Moment Estimation)**. Він поєднує переваги AdaGrad та RMSProp, забезпечуючи адаптивну швидкість навчання для кожного параметра.

#### 1.2.4. Функції втрат та Huber Loss
Для навчання обрано **Huber Loss**, яка поєднує стабільність MSE та стійкість MAE до викидів:
