# Технічний Паспорт Активу: data/DUQ_hourly.csv (GIGA-PASSPORT v3)
...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">DUQ_hourly.csv</h1>
            <p class="mega-subtitle">Локальний набір історичних даних погодинного навантаження енергосистеми Duquesne Light (Піттсбург та Західна Пенсильванія, США)</p>
...
<div class="metrics-grid">
    ...
    <div class="glass-card metric-card">
        ...
        <div class="metric-info">
            ...
            <span class="metric-value">2.68 MB</span>
        </div>
    </div>
    <div class="glass-card metric-card">
        <div class="metric-icon">🧬</div>
        <div class="metric-info">
            ...
            <span class="metric-value">Duquesne System (Pittsburgh Metro)</span>
        </div>
    </div>
    <div class="glass-card metric-card">
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>
            Набір даних <code>data/DUQ_hourly.csv</code> містить довгостроковий погодинний архів споживання електроенергії в зоні покриття оператора <b>Duquesne Light Company (DUQ)</b>. Цей регіон охоплює Піттсбурзький промисловий мегаполіс з історично високою концентрацією важкої металургії та сучасної високотехнологічної інфраструктури в західній Пенсильванії.
        </p>
...
<div class="section-container">
    ...
    <div class="glass-card">
        <div class="diagram-outer-wrapper">
            <div class="mermaid">
            graph TD
                CSV_ON_DISK[("data/DUQ_hourly.csv")] --> PD_READ["pd.read_csv(usecols, parse_dates)"]
...
# Псевдокод обробки часового ряду DUQ
import pandas as pd
import numpy as np

def preprocess_duq_dataset(file_path: str) -> pd.DataFrame:
    ...
    if df['DUQ_MW'].isnull().any():
        df['DUQ_MW'] = df['DUQ_MW'].interpolate(method='cubic')
        
    # 5. Оптимізація типів даних для мінімізації споживання RAM
    df['DUQ_MW'] = df['DUQ_MW'].astype(np.float32)
...
<div class="section-container">
    <div class="section-header"><span class="section-number">05</span><h2 class="section-title">Технічний FAQ датасету DUQ</h2></div>
    <div class="glass-card">
        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q1: Яка фізична специфіка споживання в регіоні DUQ (Duquesne Light)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Регіон покриття Duquesne Light історично відомий як важкопромислове серце Пенсильванії. Споживання характеризується високою базовою стабільністю та чіткими тижневими циклами: у робочі дні великі промислові об'єкти створюють величезне базове навантаження, яке значно падає на вихідних (Weekend Drop). Також у самому місті Піттсбург велику роль грає житлове навантаження з двома добовими піками та літня кондиціонерна сезонність.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q2: Чому для цього набору даних важливо аналізувати автокореляцію (ACF)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Функція автокореляції (ACF) для даних Duquesne виявляє дуже потужні сезонні лаги на 24 та 168 годинах, що підтверджує строгу добову та тижневу циклічність. Саме тому в моделях LSTM лаг `L(t-24)` та `L(t-168)` (значення споживання рівно добу та тиждень тому) є найважливішими вхідними ознаками, що дозволяють нейромережі точно схоплювати періодичність навантаження.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q3: Як система ATLAS справляється з переходами на літній/зимовий час (DST) у цьому датасеті?</h4>
        <p style="color: var(--text-dim);">
            A: Перехід на літній час призводить до появи "пропущеної години" (2:00 стає 3:00, отже даних за другу годину немає), а перехід на зимовий — до "дубльованої години" (коли 1:00-2:00 повторюється двічі). Наш препроцесор у <code>vectorizer.py</code> локалізує часову зону в формат `US/Eastern`, автоматично обробляє амбівалентність дублікатів через `keep='first'` та інтерполює пропущені години, відновлюючи безперервність фізичної годинної сітки без спотворення індексів.
        </p>
    </div>
</div>
