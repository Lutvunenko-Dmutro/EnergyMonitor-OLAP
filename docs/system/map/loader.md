...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">Служба автоматизації: loader</h1>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>Модуль <b>loader</b> забезпечує інтеграцію та виконання наступних обчислювальних процесів системи: <i>📦 REAL-TIME DATA LOADER (ETL Stream Handler). py | Версія: 2.3.0 Призначення: Центральний вузол управління життєвим циклом даних: від ініціалізації з'єднань до інтелектуального завантаження аналітичних зрізів.</i><br><br><b>Архітектурний інтерфейс:</b> надає методи <code>fetch_granular_data()</code>, <code>load_kaggle_lazy()</code>.</p>
    </div>
</div>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step" style="border-left: 4px solid var(--accent); padding-left: 20px;">
        <p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>📦 REAL-TIME DATA LOADER (ETL Stream Handler).</p>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'><b>py | Версія:</b> 2.3.0</p>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'><b>Призначення:</b> Центральний вузол управління життєвим циклом даних: від ініціалізації з'єднань до інтелектуального завантаження аналітичних зрізів.</p>
...
<li style='margin-bottom: 6px; line-height: 1.5;'><b>🏗️ Active Boot Sequencing:</b> Поетапне завантаження компонентів з візуальною індикацією прогресу.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>⚡ Multi-level Caching:</b> Дворівнева оптимізація доступу (RAM Cache + Session Fallback).</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>🦥 Lazy Loading:</b> Завантаження великих архівів (Kaggle) виключно за запитом (Just-in-Time).</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>🛡️ Robust Recovery:</b> Автоматичне відновлення даних та Seeder-інтеграція при порожній БД.</li>
</ul>
    </div>
</div>
    ...
    <div class="glass-card flow-step">
        ...
        <div style='display: flex; flex-direction: column; gap: 10px;'>
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def fetch_granular_data()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Завантажує конкретний підмножина даних для поетапного завантаження.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def load_kaggle_lazy()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Lazy-завантаження Kaggle CSV з кешем 5 хвилин.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def get_active_boot_data_generator()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Generator: yields (message, progress_pct, data_chunk).</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def fetch_database_data()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Повне завантаження даних (резервний варіант).</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def get_verified_data()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Головна точка входу для отримання даних з валідацією та відновленням.</p>
            </div>
            </div>
    </div>
</div>
...
graph TD
    START("Запуск loader.py") --> CONFIG("Ініціалізація оточення")
