...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">Служба автоматизації: layouts</h1>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>Модуль <b>layouts</b> забезпечує інтеграцію та виконання наступних обчислювальних процесів системи: <i>ОРКЕСТРАТОР ПРЕДСТАВЛЕННЯ ПРОГНОЗІВ ТА БЕКТЕСТІВ (Forecast & Backtest Layouts) Модуль відповідає за компоновку результатів прогнозування та управління циклами бектестування.</i><br><br><b>Архітектурний інтерфейс:</b> надає методи <code>render_single_forecast_results()</code>, <code>render_backtest_execution_loop()</code>.</p>
    </div>
</div>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step" style="border-left: 4px solid var(--accent); padding-left: 20px;">
        <p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>ОРКЕСТРАТОР ПРЕДСТАВЛЕННЯ ПРОГНОЗІВ ТА БЕКТЕСТІВ (Forecast & Backtest Layouts)</p>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>Модуль відповідає за компоновку результатів прогнозування та управління циклами бектестування.</p>
...
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Forecast Accuracy Audit:</b> інтелектуальний аналіз точності (MAPE, RMSE, R²) з індикацією статусу калібрування.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Mega-Hybrid Visualization:</b> оркестрація графіків, що об'єднують історію бектесту, прогноз та довірчі інтервали.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Backtest Loop Orchestration:</b> управління ітераційними циклами тестування (старт, пауза, зупинка) з прогрес-баром.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Academic Reporting Engine:</b> автоматична генерація академічних графіків розподілу помилок та кореляції.</li>
</ol>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>Забезпечує професійний рівень інтерпретації результатів ШІ-моделей для операторів та аналітиків.</p>
    </div>
</div>
    ...
    <div class="glass-card flow-step">
        ...
        <div style='display: flex; flex-direction: column; gap: 10px;'>
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def render_single_forecast_results()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Renders the detailed post-forecast accuracy audit and mega chart.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def render_backtest_execution_loop()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Renders the iterative background backtest processing UI.</p>
            </div>
            </div>
    </div>
</div>
...
graph TD
    START("Запуск layouts.py") --> CONFIG("Ініціалізація оточення")
