# Технічний Паспорт Активу: data/AEP_hourly.csv (GIGA-PASSPORT v3)
...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">AEP_hourly.csv</h1>
            <p class="mega-subtitle">Локальний набір історичних даних погодинного навантаження енергосистеми American Electric Power (Східна мережа США)</p>
...
<div class="metrics-grid">
    ...
    <div class="glass-card metric-card">
        ...
        <div class="metric-info">
            ...
            <span class="metric-value">3.24 MB</span>
        </div>
    </div>
    <div class="glass-card metric-card">
        <div class="metric-icon">🧬</div>
        <div class="metric-info">
            ...
            <span class="metric-value">AEP System (11 States)</span>
        </div>
    </div>
    <div class="glass-card metric-card">
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>
            Набір даних <code>data/AEP_hourly.csv</code> є первинним історичним архівом споживання активної потужності в межах об'єднання <b>American Electric Power (AEP)</b>, однієї з найбільших приватних енергетичних систем у США, яка обслуговує понад 5.5 мільйонів споживачів у 11 штатах.
        </p>
...
<div class="section-container">
    ...
    <div class="glass-card">
        <div class="diagram-outer-wrapper">
            <div class="mermaid">
            graph TD
                CSV_ON_DISK[("data/AEP_hourly.csv")] --> PD_READ["pd.read_csv(usecols, parse_dates)"]
...
# Псевдокод обробки часового ряду AEP
import pandas as pd
import numpy as np

def preprocess_aep_dataset(file_path: str) -> pd.DataFrame:
    ...
    if df['AEP_MW'].isnull().any():
        df['AEP_MW'] = df['AEP_MW'].interpolate(method='cubic')
        
    # 5. Оптимізація типів даних для мінімізації споживання RAM
    df['AEP_MW'] = df['AEP_MW'].astype(np.float32)
...
<div class="section-container">
    <div class="section-header"><span class="section-number">05</span><h2 class="section-title">Технічний FAQ датасету AEP</h2></div>
    <div class="glass-card">
        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q1: Яка фізична специфіка споживання в регіоні AEP (American Electric Power)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Оскільки східна мережа США охоплює 11 штатів з різко вираженим континентальним кліматом, часовий ряд AEP має чітку двопікову сезонність. Літній пік спричинений масовим використанням систем кондиціонування повітря (пікове навантаження у липні-серпні), а зимовий — електричним опаленням (грудень-січень).
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q2: Чому для очищення пропусків застосовується саме кубічна інтерполяція, а не заповнення попереднім значенням (FFill)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Енергоспоживання є неперервним та плавним фізичним процесом з циклічною поведінкою. Заповнення попереднім значенням (FFill) або лінійна інтерполяція створюють різкі кутові злами в точках стику, що негативно впливає на якість розрахунку градієнтів у нейромережах LSTM. Кубічна інтерполяція будує гладкий кубічний сплайн, що зберігає фізичну безперервність першої похідної.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q3: Як уникнути витоку даних (Data Leakage) при навчанні на цьому файлі?</h4>
        <p style="color: var(--text-dim);">
            A: При підготовці ознак за допомогою <code>vectorizer.py</code> всі операції масштабування (MinMax або StandardScaler) вираховуються строго на тренувальному підмножині (наприклад, перші 80% ряду), після чого отримані коефіцієнти $\mu$ та $\sigma$ застосовуються до тестової вибірки. Це запобігає проникненню майбутньої інформації про розподіл навантаження в модель на етапі навчання.
        </p>
    </div>
</div>
