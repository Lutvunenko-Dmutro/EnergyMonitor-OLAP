# Технічний Паспорт Активу: data/EKPC_hourly.csv (GIGA-PASSPORT v3)
...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">EKPC_hourly.csv</h1>
            <p class="mega-subtitle">Локальний набір історичних даних погодинного навантаження енергосистеми East Kentucky Power Cooperative (Кентуккі, США)</p>
...
<div class="metrics-grid">
    ...
    <div class="glass-card metric-card">
        ...
        <div class="metric-info">
            ...
            <span class="metric-value">1.18 MB</span>
        </div>
    </div>
    <div class="glass-card metric-card">
        <div class="metric-icon">🧬</div>
        <div class="metric-info">
            ...
            <span class="metric-value">EKPC System (Rural Kentucky)</span>
        </div>
    </div>
    <div class="glass-card metric-card">
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>
            Набір даних <code>data/EKPC_hourly.csv</code> містить довгостроковий погодинний архів споживання електроенергії в зоні покриття оператора <b>East Kentucky Power Cooperative (EKPC)</b>. Ця кооперативна мережа обслуговує понад 1.1 мільйона людей у 87 округах штату Кентуккі, маючи виражений сільський та житлово-комунальний характер споживання з високою залежністю від аграрних та кліматичних циклів.
        </p>
...
<div class="section-container">
    ...
    <div class="glass-card">
        <div class="diagram-outer-wrapper">
            <div class="mermaid">
            graph TD
                CSV_ON_DISK[("data/EKPC_hourly.csv")] --> PD_READ["pd.read_csv(usecols, parse_dates)"]
...
# Псевдокод обробки часового ряду EKPC
import pandas as pd
import numpy as np

def preprocess_ekpc_dataset(file_path: str) -> pd.DataFrame:
    ...
    if df['EKPC_MW'].isnull().any():
        df['EKPC_MW'] = df['EKPC_MW'].interpolate(method='cubic')
        
    # 5. Оптимізація типів даних для мінімізації споживання RAM
    df['EKPC_MW'] = df['EKPC_MW'].astype(np.float32)
...
<div class="section-container">
    <div class="section-header"><span class="section-number">05</span><h2 class="section-title">Технічний FAQ датасету EKPC</h2></div>
    <div class="glass-card">
        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q1: Яка фізична специфіка споживання в регіоні EKPC (East Kentucky Power Cooperative)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Оскільки EKPC обслуговує переважно сільську місцевість штату Кентуккі, тут майже відсутнє велике промислове навантаження, але надзвичайно сильно виражений житловий та фермерський фактори. Це спричиняє величезний пік споживання взимку через електричне опалення приватних будинків. Зимовий пік навантаження в EKPC часто перевищує літній кондиціонерний пік на 30-40%, що робить прогнозування екстремальних морозів критично важливим для стабільності кооперативної мережі.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q2: Чому для цього невеликого датасету (1.18 MB) важливо налаштовувати Learning Rate Scheduler?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Зменшена кількість навчальних точок приводить до того, що великий Learning Rate наприкінці навчання змусить ваги LSTM хаотично коливатися навколо мінімуму функції втрат, заважаючи точній збіжності. Використання <code>ReduceLROnPlateau</code> з коефіцієнтом `factor=0.5` та `patience=3` динамічно знижує крок навчання, дозволяючи моделі акуратно локалізувати глобальний мінімум на невеликих сільських датасетах Кентуккі.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q3: Як сільськогосподарська специфіка Кентуккі відбивається на часовому ряді споживання?</h4>
        <p style="color: var(--text-dim);">
            A: У Кентуккі сільськогосподарські цикли (робота елеваторів, систем вентиляції ферм, сушіння зерна) створюють додаткові сезонні піки споживання восени (вересень-жовтень). Ці піки не пов'язані напряму з температурою повітря, тому для їх точного моделювання векторний процесор <code>vectorizer.py</code> впроваджує ознаку місяця року та дня року, що дозволяє LSTM-моделі розпізнавати щорічну аграрну циклічність.
        </p>
    </div>
</div>
