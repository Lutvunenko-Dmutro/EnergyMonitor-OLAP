...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">Служба автоматизації: sensors_db</h1>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>Модуль <b>sensors_db</b> забезпечує інтеграцію та виконання наступних обчислювальних процесів системи: <i>ЕМУЛЯТОР ТРАНСЛЯЦІЇ ТЕЛЕМЕТРІЇ (Digital Twin Real-Time Engine) Реалізує фоновий процес (Subprocess) для симуляції потокової телеметрії об'єктів.</i><br><br><b>Архітектурний інтерфейс:</b> надає методи <code>run_cosmetic_collector()</code>.</p>
    </div>
</div>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step" style="border-left: 4px solid var(--accent); padding-left: 20px;">
        <p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>ЕМУЛЯТОР ТРАНСЛЯЦІЇ ТЕЛЕМЕТРІЇ (Digital Twin Real-Time Engine)</p>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>Реалізує фоновий процес (Subprocess) для симуляції потокової телеметрії об'єктів.</p>
<h3 style='color: var(--accent); font-family: "Orbitron", sans-serif; font-size: 15px; margin-top: 22px; margin-bottom: 10px;'>Це серце "Цифрового двійника" системи, що забезпечує</h3>
<ol style='margin-left: 20px; margin-bottom: 15px;'>
<li style='margin-bottom: 6px; line-height: 1.5;'>Генерацію реалістичних показників (MW, Hz, H2, Temp) кожні 5 секунд.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'>Singleton-захист через PID-lock для запобігання дублювання процесів.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'>Передачу даних через швидкий JSON-буфер (live_state.json) для UI.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'>Автономний Lifecycle з механізмом Heartbeat та автоматичним самозавершенням.</li>
</ol>
    </div>
</div>
    ...
    <div class="glass-card flow-step">
        ...
        <div style='display: flex; flex-direction: column; gap: 10px;'>
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def run_cosmetic_collector()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Стабільний генератор для захисту диплому. </p>
            </div>
            </div>
    </div>
</div>
...
graph TD
    START("Запуск sensors_db.py") --> CONFIG("Ініціалізація оточення")
