...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">Служба автоматизації: memory_helper</h1>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>Модуль <b>memory_helper</b> забезпечує інтеграцію та виконання наступних обчислювальних процесів системи: <i>МОНІТОРИНГ ТА КЕРУВАННЯ РЕСУРСАМИ (Resource Watchdog) Забезпечує стабільність роботи додатка в умовах жорстких обмежень на використання RAM.</i><br><br><b>Архітектурний інтерфейс:</b> надає методи <code>get_memory_usage()</code>, <code>get_top_objects()</code>.</p>
    </div>
</div>
...
<div class="section-container">
    ...
    <div class="glass-card flow-step" style="border-left: 4px solid var(--accent); padding-left: 20px;">
        <p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>МОНІТОРИНГ ТА КЕРУВАННЯ РЕСУРСАМИ (Resource Watchdog)</p>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>Забезпечує стабільність роботи додатка в умовах жорстких обмежень на використання RAM.</p>
...
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Real-time Monitoring:</b> вимірювання фізичного споживання пам'яті (RSS) через psutil.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Proactive Cleanup:</b> механізм автоматичного скидання кешу та збору сміття (Auto-GC) при досягненні порогів.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>Session State Audit:</b> аналіз об'єктів у Streamlit Session State для запобігання Memory Leaks.</li>
<li style='margin-bottom: 6px; line-height: 1.5;'><b>DataFrame Optimization:</b> детальна діагностика споживання пам'яті колонками великих датасетів.</li>
</ol>
<p style='line-height: 1.7; margin-bottom: 12px; color: var(--text-main);'>Гарантує безперебійну роботу системи на хмарних платформах з обмеженими лімітами.</p>
    </div>
</div>
    ...
    <div class="glass-card flow-step">
        ...
        <div style='display: flex; flex-direction: column; gap: 10px;'>
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def get_memory_usage()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Повертає споживання RAM поточним процесом у MB.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def get_top_objects()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Аналізує st.session_state та повертає топ-3 великих об'єкти.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def get_resource_status()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Повертає статус споживання ресурсів та топ-об'єкти.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def auto_gc()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Автоматичне очищення пам'яті при перевищенні порогу.</p>
            </div>
            
            <div style='background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 12px; border-radius: 8px;'>
                <code style='color: var(--accent); font-size: 13px; font-weight: 600;'>def df_memory_report()</code>
                <p style='margin: 4px 0 0 0; font-size: 12.5px; color: var(--text-dim);'>Повертає рядок зі статистикою пам'яті DataFrame.</p>
            </div>
            </div>
    </div>
</div>
...
graph TD
    START("Запуск memory_helper.py") --> CONFIG("Ініціалізація оточення")
