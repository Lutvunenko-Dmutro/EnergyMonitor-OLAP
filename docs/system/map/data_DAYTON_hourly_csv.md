# Технічний Паспорт Активу: data/DAYTON_hourly.csv (GIGA-PASSPORT v3)
...
<div class="hero-section">
    ...
    <div class="hero-main">
        ...
        <div class="hero-title-group">
            <h1 class="mega-title">DAYTON_hourly.csv</h1>
            <p class="mega-subtitle">Локальний набір історичних даних погодинного навантаження енергосистеми Dayton Power & Light (Західне Огайо, США)</p>
...
<div class="metrics-grid">
    ...
    <div class="glass-card metric-card">
        ...
        <div class="metric-info">
            ...
            <span class="metric-value">2.75 MB</span>
        </div>
    </div>
    <div class="glass-card metric-card">
        <div class="metric-icon">🧬</div>
        <div class="metric-info">
            ...
            <span class="metric-value">DP&L System (West Ohio)</span>
        </div>
    </div>
    <div class="glass-card metric-card">
...
<div class="section-container">
    ...
    <div class="glass-card flow-step">
        <p>
            Набір даних <code>data/DAYTON_hourly.csv</code> містить довгостроковий погодинний архів споживання електроенергії в зоні покриття оператора <b>Dayton Power & Light (DP&L)</b>, що обслуговує житлові, комерційні та індустріальні райони у західній частині штату Огайо.
        </p>
...
<div class="section-container">
    ...
    <div class="glass-card">
        <div class="diagram-outer-wrapper">
            <div class="mermaid">
            graph TD
                CSV_ON_DISK[("data/DAYTON_hourly.csv")] --> PD_READ["pd.read_csv(usecols, parse_dates)"]
...
# Псевдокод обробки часового ряду DAYTON
import pandas as pd
import numpy as np

def preprocess_dayton_dataset(file_path: str) -> pd.DataFrame:
    ...
    if df['DAYTON_MW'].isnull().any():
        df['DAYTON_MW'] = df['DAYTON_MW'].interpolate(method='cubic')
        
    # 5. Оптимізація типів даних для мінімізації споживання RAM
    df['DAYTON_MW'] = df['DAYTON_MW'].astype(np.float32)
...
<div class="section-container">
    <div class="section-header"><span class="section-number">05</span><h2 class="section-title">Технічний FAQ датасету DAYTON</h2></div>
    <div class="glass-card">
        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q1: Яка фізична специфіка споживання в регіоні DAYTON (Dayton Power & Light)?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Зона DP&L має помірну щільність навантаження та збалансовану житлово-комунальну структуру. Добовий хід навантаження має виражений ранковий пік (о 8:00-9:00, коли споживачі збираються на роботу) та вечірній пік (о 19:00-21:00). Сезонність тісно пов'язана з погодними факторами штату Огайо: високі навантаження спостерігаються під час літньої спеки (липень) та зимових морозів (січень).
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q2: Чому нормалізація MinMax є критичною для моделі LSTM при навчанні на даних Dayton?</h4>
        <p style="color: var(--text-dim); margin-bottom: 15px;">
            A: Оскільки абсолютні значення навантаження Dayton вимірюються в тисячах мегават (від 1000 до 3500 MW), пряма подача таких великих чисел у нейронну мережу з функціями активації Sigmoid або Tanh призведе до насичення градієнтів та зупинки навчання (Gradient Saturation). Масштабування MinMax стискає дані в діапазон $[0, 1]$, забезпечуючи стабільну роботу градієнтного спуску та швидку збіжність моделі.
        </p>

        <h4 style="color: var(--text-main); margin-bottom: 5px;">Q3: Як врахувати святкові та вихідні дні (Holidays effect) для підвищення точності прогнозу на даних Dayton?</h4>
        <p style="color: var(--text-dim);">
            A: Векторизатор <code>vectorizer.py</code> додає бінарну ознаку <code>is_holiday</code> (за допомогою бібліотеки <code>holidays</code> для США) та категоріальну ознаку дня тижня (Day of Week). У святкові дні (наприклад, День Подяки або Різдво) поведінка споживання у Дейтоні наближається до недільного профілю, що дозволяє ШІ уникнути завищення прогнозів навантаження у ці періоди.
        </p>
    </div>
</div>
