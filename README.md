...
| Версія (Version) | Кількість Ознак | Вихідні таргети | Особливості |
| :---: | :---: | :---: | :--- |
| **`v1`** | 1 (`load_mw`) | 1 (`load_mw`) | Базова модель одновимірного часового ряду. |
| **`v2`** | 5 (v1 + Погода) | 2 (`load_mw`, `health`) | Багатофакторна рекурентна мережа. |
| **`v3`** | 9 (v2 + Час) | 2 (`load_mw`, `health`) | **Релізна версія**: враховує сезонність. |
| **`Zero-Fail`**| - | 2 (`load_mw`, `health`) | **Fallback Mode**: Seasonal Naive при відсутності ШІ. |
...
*   **Параметризація запитів:** Захист від SQL-ін'єкцій реалізований на рівні ORM-моделей `SQLAlchemy` та спеціалізованого модуля `validators.py`.
*   **Система Діагностики:** Проект містить `diagnose.py` — двигун автоматичного аудиту безпеки та якості коду (Antigravity Diagnostics v2.0).
*   **Zero-Failure Fallback:** У разі збою AI-обчислень система автоматично переходить на статистичний бейзлайн (Seasonal Naive), забезпечуючи безперервність моніторингу.
*   **Configuration Isolation:** Усі секрети ізольовані у файлах `.env` ( Twelve-Factor App).
